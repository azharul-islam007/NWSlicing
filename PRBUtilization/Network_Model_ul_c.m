classdef Network_Model_ul_c < Network_Model_c
    ...
    methods
        function self = Network_Model_ul_c(M_eMBB_i, M_veh_i, M_mMTC_i, lambda_e_i, sim_nr_drops_per_round_i, sim_nr_round_i)
            ...
            self.nw_characteristics_v2x_CL = struct('SINRdB', zeros(self.sim_nr_drops_per_round, self.M_veh), 'nr_packets', zeros(self.sim_nr_drops_per_round, self.M_veh));
            self.nw_characteristics_v2x_SL = struct('SINRdB', zeros(self.sim_nr_drops_per_round, self.M_veh), 'nr_packets', zeros(self.sim_nr_drops_per_round, self.M_veh));
        ...
        function [pd_v2x_CL_SINR_dB_o, pd_v2x_CL_Nr_packet_o, ...
            ...
            if (self.M_veh_UE_cellular > 1) % To fit the normal distribution, at least two elements are provided
                sinr_dB = reshape(self.nw_characteristics_v2x_CL.SINRdB, [], 1);
                pd_v2x_CL_SINR_dB_o = fitdist(sinr_dB, 'Normal'); 
                nr_packets = reshape(self.nw_characteristics_v2x_CL.nr_packets, [], 1);            
             ...
             if (self.M_veh_UE_sidelink > 1) % To fit the normal distribution, at least two elements are provided
                sinr_dB = reshape(self.nw_characteristics_v2x_SL.SINRdB, [], 1);
                pd_v2x_SL_SINR_dB_o = fitdist(sinr_dB, 'Normal'); 
                nr_packets = reshape(self.nw_characteristics_v2x_SL.nr_packets, [], 1);            
