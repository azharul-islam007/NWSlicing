function [Rb_o, Speff_o] = offline_nw_eMBB_UL_f(Avg_T_i, ...
    M_eMBB_i, ...
    pd_nr_session_i, ...
    pd_SINR_dB_i ...
    )
...
for t = 1 : Avg_T
    ...
    for m = 1 : M_eMBB
        Rb{t}{m} = 1e6 * pd_nr_session{m}.random(); % one session requires 1Mbps
        SINR_dB = pd_SINR_dB{m}.random();
        Speff{t}{m} = mapping_SINR2Speff_f(SINR_dB, 'ul');
    end
end
