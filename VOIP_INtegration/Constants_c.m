classdef Constants_c < handle
    properties (Constant)
        ...
        lambda_e_lst = [300] %300 : 100 : 600;            

        % Number of mMTC UE (including VoIP UE) randomly distributed in the cell
        M_mMTC = 120; 

        % percentage of VoIP UE among mMTC UE
        p_voip_mMTC = 1/4;

        % the wakeup-transmission probability of an mMTC packet
        p_mMTC_tx = 0.05;  

        % the voice speaking probability of a VoIP UE
        p_voip_tx = 0.95;

        % the max packet size of mMTC packets (bytes)
        max_mMTC_packet_size = 128;     

        % voip packet size (bytes), up-rounded to the nearest power of 2
        voip_packet_size = 2^ceil(log2(856));

        % voip periodicity (slot)
        void_periodicity = 20;
        
        % number of simulation drop in each round
        sim_nr_drops_per_round = 10; 
    ...
    methods(Static)
        function [] = print_configurations()
            ...
            fprintf(Constants_c.fid, 'p_voip_mMTC = %.3f\n', Constants_c.p_voip_mMTC);
            fprintf(Constants_c.fid, 'p_mMTC_tx = %f\n', Constants_c.p_mMTC_tx);  
            fprintf(Constants_c.fid, 'p_voip_tx = %f\n', Constants_c.p_voip_tx);  
            fprintf(Constants_c.fid, 'max_mMTC_packet_size = %d\n', Constants_c.max_mMTC_packet_size);   
            fprintf(Constants_c.fid, 'voip_packet_size = %d\n', Constants_c.voip_packet_size);  
