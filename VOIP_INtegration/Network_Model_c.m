classdef Network_Model_c < handle
    properties
        ...
        pd_mMTC_tx;
        % the dsitribution of the speech active probability (i.e. the
        % probability of an VoIP UE to speak)
        pd_voip_tx;
        ...
        nw_characteristics_v2x; % network characteristics for V2X service
        nw_characteristics_eMBB; % network characteristics for eMBB service
        nw_characteristics_mMTC; % network characteristics for mMTC service

        nw_outage_v2x; % network outage rate for V2X service in one round
        nw_outage_eMBB; % network outage rate for eMBB service in one round
        nw_outage_mMTC; % network outage rate for mMTC service in one round
        nw_outage; % network outage rate of all services in one round

        prb_utilization_ratio_v2x; % PRB utilization for V2X service in one round
        prb_utilization_ratio_eMBB; % PRB utilization for eMBB service in one round
        prb_utilization_ratio_mMTC; % PRB utilization for mMTC service in one round
        prb_utilization_ratio; % PRB utilization for all services
    end
    ...
    methods
        function self = Network_Model_c(M_eMBB_i, M_veh_i, M_mMTC_i, lambda_e_i, sim_nr_drops_per_round_i)
            ...
            p_voip_tx = Constants_c.p_voip_tx;
            self.pd_voip_tx = makedist('Binomial', 'N', 1, 'p', p_voip_tx);  
        ...
        function [] = init_simulation(self)
            ...
            % The first M_voip UE in the front of self.mMTC_UEs are
            % dedicated for VoIP UEs
            M_voip = round(self.M_mMTC * Constants_c.p_voip_mMTC);
            for count = 1 : self.M_mMTC
                ...
                if count <= M_voip
                    packet_size = Constants_c.voip_packet_size;
                    tx_flag = self.pd_voip_tx.random();
                    periodicity = Constants_c.void_periodicity;
                    offset = randsample(1, periodicity, 1);
                    self.mMTC_UEs{count} = voip_UE_c(self.next_ueId, x, y, ...
                        Constants_c.UE_height, packet_size, tx_flag, periodicity, offset);
                else
        ...
        function [] = prepare_simulation_round(self, r_i)
			% prepare the network characteristics structures
            self.nw_characteristics_v2x = struct();
            self.nw_characteristics_v2x.SINRdB = cell(self.sim_nr_drops_per_round, 1);
            self.nw_characteristics_v2x.nr_packets = cell(self.sim_nr_drops_per_round, 1);
			...
			% prepare the network outage rates
            ...
            self.nw_outage = zeros(self.sim_nr_drops_per_round, 1);

			% prepare the PRB utilization ratios
        ...
        function [] = prepare_simulation_drop(self, t_i)
            ...
            M_voip = round(self.M_mMTC * Constants_c.p_voip_mMTC);
            for count = 1 : self.M_mMTC
                % It is assuming that, the first M_voip UEs in front of mMTC UEs are VOIP UE
                if count <= M_voip
                    % to check the voip transmission opportunity
                    if (mod(t_i, self.mMTC_UEs{count}.periodicity) == self.mMTC_UEs{count}.offset)
                        self.mMTC_UEs{count}.tx_flag = self.pd_voip_tx.random();
                    else
                        self.mMTC_UEs{count}.tx_flag = 0;
                    end
                else
            ...
            % reset the nw characterisitcs
            self.nw_characteristics_v2x.SINRdB{t_i} = cell(self.M_veh, 1);
            self.nw_characteristics_v2x.nr_packets{t_i} = cell(self.M_veh, 1);

            self.nw_characteristics_eMBB.SINRdB{t_i} = cell(self.M_eMBB, 1);
            self.nw_characteristics_eMBB.nr_session{t_i} = cell(self.M_eMBB, 1);

            self.nw_characteristics_mMTC.SINRdB{t_i} = cell(self.M_mMTC, 1);
            self.nw_characteristics_mMTC.packet_size{t_i} = cell(self.M_mMTC, 1);
            self.nw_characteristics_mMTC.tx_flags{t_i} = cell(self.M_mMTC, 1);
        ...
        function [] = calculate_PRB_utilization(self, t_i, dl_or_ul_i)
			...
			% Log the events of the outage of V2X service
            if (Gamma1 > Gamma1_max)
                self.nw_outage_v2x(t_i) = 1;
            else
                self.nw_outage_v2x(t_i) = 0;
            end
			...
			% Log the event for the outage of eMBB service
            if (Gamma2 > Gamma2_max)
                self.nw_outage_eMBB(t_i) = 1;
            else
                self.nw_outage_eMBB(t_i) = 0;
            end
			...
			% Log the event for the outage of mMTC service
            if (Gamma3 > Gamma3_max)
                self.nw_outage_mMTC(t_i) = 1;
            else
                self.nw_outage_mMTC(t_i) = 0;
            end

			% Log the event if any outage occurs in any one of the service
            if (self.nw_outage_v2x(t_i) + self.nw_outage_eMBB(t_i) + self.nw_outage_mMTC(t_i) >= 1)
                self.nw_outage(t_i) = 1;
            else
                self.nw_outage(t_i) = 0;
            end
            ...
            self.prb_utilization_ratio(t) = (nr_PRB_v2x + nr_PRB_eMBB + nr_PRB_mMTC) / (Gamma1_max + Gamma2_max + Gamma3_max);

            fprintf(Constants_c.fid, 'PRB utilization at t=%d: %f/%f, %f/%f, %f/%f\n', t_i, nr_PRB_v2x, Gamma1_max, nr_PRB_eMBB, Gamma2_max, nr_PRB_mMTC, Gamma3_max);
        end

        function [v2x_o, eMBB_o, mMTC_o, aggregate_o] = calculate_avg_PRB_utilization(self)
			% calculate PRB utilization for each type of services
            v2x_o = mean(self.prb_utilization_ratio_v2x);
            eMBB_o = mean(self.prb_utilization_ratio_eMBB);
            mMTC_o = mean(self.prb_utilization_ratio_mMTC);
            aggregate_o = mean(self.prb_utilization_ratio);
        end

		
        function [v2x_o, eMBB_o, mMTC_o, aggregate_o] = calculate_avg_outage_rate(self)
			% calculate average outage rate separately for each services
            v2x_o = mean(self.nw_outage_v2x);
            eMBB_o = mean(self.nw_outage_eMBB);
            mMTC_o = mean(self.nw_outage_mMTC);
            aggregate_o = mean(self.nw_outage);
        end        
